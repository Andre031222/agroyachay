# Configuración de Hardware — ESP32 + DHT11 + FC-28

> English version: [`HARDWARE_SETUP.md`](HARDWARE_SETUP.md)
...
|---|---|---|
| Microcontrolador | ESP32 DevKit v1 | Procesa sensores y envía datos por WiFi |
...
3. El portal de configuración abre en `http://192.168.4.1`.
4. Seleccionar la red WiFi, ingresar contraseña y la URL del servidor.
...
Mantener presionado el botón BOOT 5 segundos. El LED parpadeará 5 veces y el ESP32
borrará las credenciales y volverá al modo Setup.
...
El DHT11 no es calibrable por software más allá de un offset fijo. Si el valor real
difiere del sensor, ajustar en el firmware:
...
El FC-28 lee conductividad del suelo. Más humedad = mayor voltaje = mayor valor
ADC (0-4095).
...
2. Sumergir las puntas en agua (no el módulo) — anotar el `raw ADC` — poner `SOIL_WET = valor - 50`.
...
5. El sistema escanea redes WiFi y configura el ESP32 automáticamente.
...
| GET | /api/sensores/esp32/:id/estado | Estado y últimas lecturas |
...
| POST | /api/serial/configurar | Envía config WiFi al ESP32 por USB |
