...
export default function ModalVincular({ sensor, cultivos, onClose, onSuccess }) {
  ...
  const handleVincular = async () => {
    if (!cultivoId) { setError(t('devices.selectCrop')); return; }
    setLoading(true);
    setError('');
    try {
      ...
      if (res.data.success) {
        ...
        setError(res.data.message || t('devices.linkError'));
      }
    } catch {
      ...
      <div className="bg-white dark:bg-gray-900 rounded-[calc(1rem-0.375rem)] ring-1 ring-black/5 dark:ring-white/5 overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-black/5 dark:border-white/10">
          <div className="flex items-center gap-2.5">
            ...
            <h3 className="font-semibold tracking-tight text-gray-900 dark:text-white text-sm">{t('devices.linkDeviceTitle')}</h3>
          </div>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-gray-600 dark:text-gray-300 mb-1.5">
              {t('devices.assignToCrop')} <span className="text-red-400">*</span>
            </label>
            <select
              value={cultivoId}
              ...
              className={fieldClass}
            >
              <option value="">{t('devices.selectCropOption')}</option>
              {cultivos.map(c => (
        ...
        <div className="flex gap-3 px-6 py-4 border-t border-black/5 dark:border-white/10">
          ...
          >
            ...
            {loading ? t('devices.linking') : t('devices.link')}
          </button>
        </div>
      </div>
      </div>
