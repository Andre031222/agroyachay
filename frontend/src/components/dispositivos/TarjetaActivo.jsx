...
function TarjetaActivo({ sensor, onConfigurar, onDesvincular }) {
  ...
  const estadoText = sensor.estado ? t(`devices.status.${sensor.estado}`) : '—';
    ...
    <div className="group rounded-2xl p-1.5 bg-gray-50/60 dark:bg-white/[0.03] ring-1 ring-black/5 dark:ring-white/10 transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] hover:-translate-y-0.5">
      ...
      <div className="flex items-start justify-between mb-4">
        ...
        <span className={`flex items-center gap-1.5 shrink-0 ml-2 text-xs font-semibold px-2.5 py-1 rounded-full ring-1 ${
          ...
          {conectado ? t('devices.online') : t('devices.offline')}
        </span>
      </div>
      ...
      <div className="space-y-1.5 mb-4">
        {sensor.cultivo_nombre && (
          <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
            ...
            <span className="truncate">{t('devices.crop')}: <strong className="text-gray-700 dark:text-gray-300">{sensor.cultivo_nombre}</strong></span>
          </div>
        )}
        ...
        <div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400">
          ...
          <span>{t('devices.lastSignal')}: <strong className="text-gray-700 dark:text-gray-300">{tiempoRelativo(sensor.fecha_ultima_lectura, intlLocale, t('devices.noData'))}</strong></span>
        </div>
      </div>
