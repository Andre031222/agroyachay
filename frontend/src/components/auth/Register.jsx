...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        notify.success(t('auth.welcomeNamed', { name: formData.nombre }));
    ...
    <div className="min-h-[100dvh] flex flex-col items-center justify-center px-4 py-8 bg-gray-50 dark:bg-gray-950">
      ...
      <div className="w-full max-w-[420px] animate-slide-up">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">{t('auth.createFreeAccount')}</p>
        </div>
        ...
        <div className="rounded-2xl bg-white dark:bg-gray-900 ring-1 ring-black/5 dark:ring-white/10 p-7">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label htmlFor="nombre" className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">{t('auth.name')}</label>
                <input id="nombre" type="text" name="nombre" value={formData.nombre}
                  onChange={handleChange} placeholder={t('auth.namePlaceholder')} required className={fieldClass} />
              </div>
              <div>
                <label htmlFor="email" className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">{t('auth.email')}</label>
                <input id="email" type="email" name="email" value={formData.email}
                  onChange={handleChange} placeholder={t('auth.emailPlaceholder')} required className={fieldClass} />
              </div>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label htmlFor="password" className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">{t('auth.password')}</label>
                <div className="relative">
                  <input id="password" type={showPwd ? 'text' : 'password'} name="password"
                    value={formData.password} onChange={handleChange}
                    placeholder={t('auth.minCharsPlaceholder')} required className={fieldClass + ' pr-11'} />
              ...
              <div>
                <label htmlFor="confirm" className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">{t('auth.confirm')}</label>
                <input id="confirm" type="password" name="confirm" value={formData.confirm}
                  onChange={handleChange} placeholder={t('auth.repeatPassword')} required className={fieldClass} />
              </div>
            </div>
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? <><Spinner /> {t('auth.creatingAccount')}</> : t('auth.createAccountCta')}
            </button>
          </form>
        </div>
