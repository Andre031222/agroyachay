...
const ESPECIALIDADES = [
  { tipo:'Nutrición Vegetal',      icon: Ic.leaf,   expertos:12, tiempo:'24h', iconBg:'bg-emerald-500', light:'bg-emerald-50 dark:bg-emerald-900/20', text:'text-emerald-700 dark:text-emerald-400' },
  { tipo:'Manejo Hídrico',         icon: Ic.drop,   expertos:8,  tiempo:'24h', iconBg:'bg-sky-500',     light:'bg-sky-50 dark:bg-sky-900/20',         text:'text-sky-700 dark:text-sky-400' },
  { tipo:'Manejo de Suelos',       icon: Ic.sprout, expertos:10, tiempo:'48h', iconBg:'bg-amber-500',   light:'bg-amber-50 dark:bg-amber-900/20',     text:'text-amber-700 dark:text-amber-400' },
  { tipo:'Control Fitosanitario',  icon: Ic.bug,    expertos:15, tiempo:'12h', iconBg:'bg-red-500',     light:'bg-red-50 dark:bg-red-900/20',         text:'text-red-700 dark:text-red-400' },
  { tipo:'Análisis Productivo',    icon: Ic.trend,  expertos:9,  tiempo:'36h', iconBg:'bg-violet-500',  light:'bg-violet-50 dark:bg-violet-900/20',   text:'text-violet-700 dark:text-violet-400' },
  { tipo:'Agricultura Sostenible', icon: Ic.shield, expertos:11, tiempo:'24h', iconBg:'bg-teal-500',    light:'bg-teal-50 dark:bg-teal-900/20',       text:'text-teal-700 dark:text-teal-400' },
];
...
const AsesoriaEspecializada = () => {
  ...
  const handlePreGroq = async () => {
    if (!form.descripcion_problema.trim() || form.descripcion_problema.trim().length < 20) {
      notify.warning(t('advisory.describeMore'));
      return;
    }
    ...
    try {
      const res = await asistenteAPI.consulta(
        t('advisory.aiPrompt', {
          topic: form.tipo_asesoria || t('advisory.yourCrop'),
          problem: form.descripcion_problema,
        }),
        { region: 'Puno, Perú' }
      );
      if (res.data.success) setPreGroq(res.data.data.respuesta);
    } catch {
      notify.error(t('advisory.aiError'));
    } finally {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.tipo_asesoria)               { notify.warning(t('advisory.selectType')); return; }
    if (!form.descripcion_problema.trim()) { notify.warning(t('advisory.describeProblem')); return; }
    setSubmitting(true);
    try {
      const res = await asesoriaAPI.solicitar(form);
      notify.success(res.data.message || t('advisory.sent'));
      setShowModal(false);
      setForm({ tipo_asesoria:'', descripcion_problema:'', prioridad:'Media' });
      setPreGroq(null);
      fetchSolicitudes();
    } catch (err) {
      notify.error(err.response?.data?.message || t('advisory.sendError'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="px-4 sm:px-6 py-6 space-y-5 bg-gray-50 dark:bg-gray-950 min-h-full">
      ...
      <div className="grid grid-cols-3 gap-3 sm:gap-4 animate-slide-up">
        {[
          { icon: Ic.msg,   val: stats.total > 0 ? stats.total : '65+',         lbl: stats.total > 0 ? t('advisory.statMyQueries') : t('advisory.statExperts'), color:'bg-violet-500' },
          { icon: Ic.check, val: stats.total > 0 ? stats.respondidas : '12-48h', lbl: stats.total > 0 ? t('advisory.statAnsweredByAI') : t('advisory.statResponseTime'), color:'bg-emerald-500' },
          { icon: Ic.clock, val: stats.total > 0 ? stats.pendientes  : '98%',    lbl: stats.total > 0 ? t('advisory.statPending') : t('advisory.statSatisfaction'), color: stats.pendientes > 0 ? 'bg-amber-500' : 'bg-sky-500' },
      ...
      <div className="rounded-[1.75rem] p-1.5 bg-gray-100/70 dark:bg-white/[0.04] ring-1 ring-black/5 dark:ring-white/10 animate-slide-up">
        <div className="rounded-[calc(1.75rem-0.375rem)] bg-white dark:bg-gray-900 p-5 sm:p-6">
          <div className="flex items-center justify-between mb-5">
            <div>
              <h2 className="font-display text-base font-bold tracking-tight text-gray-900 dark:text-white">{t('advisory.specialtiesTitle')}</h2>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{t('advisory.specialtiesHint')}</p>
            </div>
          </div>
            ...
            {ESPECIALIDADES.map(esp => (
              <button key={esp.tipo} type="button"
                ...
                <div className="min-w-0 flex-1">
                  <p className={`text-sm font-semibold tracking-tight ${esp.text}`}>{t(`advisory.specialties.${esp.tipo}.label`)}</p>
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 line-clamp-2">{t(`advisory.specialties.${esp.tipo}.desc`)}</p>
                  <div className="flex items-center gap-3 mt-2.5 text-[10px] text-gray-400">
                    <span className="flex items-center gap-1">{Ic.user} {t('advisory.expertsCount', { count: esp.expertos })}</span>
      ...
      <div className="rounded-[1.75rem] p-1.5 bg-gray-100/70 dark:bg-white/[0.04] ring-1 ring-black/5 dark:ring-white/10 animate-slide-up">
        <div className="rounded-[calc(1.75rem-0.375rem)] bg-white dark:bg-gray-900 p-5 sm:p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h2 className="font-display text-base font-bold tracking-tight text-gray-900 dark:text-white">{t('advisory.myRequests')}</h2>
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">{t('advisory.myRequestsHint')}</p>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="text-center py-12 bg-gray-50/80 dark:bg-white/[0.03] rounded-2xl ring-1 ring-black/5 dark:ring-white/10">
              ...
              <p className="font-display text-base font-bold tracking-tight text-gray-700 dark:text-gray-200 mb-1">{t('advisory.empty')}</p>
              <p className="text-xs text-gray-400 dark:text-gray-500 mb-5">{t('advisory.emptyHint')}</p>
              <button onClick={() => openModal()}
              ...
              {solicitudesFiltradas.map(sol => (
                <div key={sol.id} className="rounded-2xl p-4 ring-1 ring-black/5 dark:ring-white/10 bg-gray-50/60 dark:bg-white/[0.02] transition-all duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] hover:-translate-y-0.5">
                  <div className="flex items-start justify-between gap-3 mb-2 flex-wrap">
                    <div>
                      <p className="font-semibold tracking-tight text-sm text-gray-900 dark:text-white">{t(`advisory.specialties.${sol.tipo_asesoria}.label`)}</p>
                      <div className="flex items-center gap-2 mt-1.5">
                        <span className={`text-[10px] font-semibold px-2.5 py-0.5 rounded-full ring-1 ring-black/5 dark:ring-white/10 ${estadoBadge(sol.estado)}`}>
                          {t(`advisory.statuses.${sol.estado}`)}
                        </span>
                        <span className={`text-[10px] font-semibold px-2.5 py-0.5 rounded-full ring-1 ring-black/5 dark:ring-white/10 ${PRIORIDADES.find(p => p.val === sol.prioridad)?.cls || ''}`}>
                          {t(`advisory.priorities.${sol.prioridad}`)}
                        </span>
                      </div>
                    </div>
                    <span className="text-xs text-gray-400 flex items-center gap-1 shrink-0">
                      {Ic.clock}
                      {formatDate(sol.fecha_solicitud, { day:'numeric', month:'short', year:'numeric' })}
                    </span>
                  </div>
                    ...
                    return (
                      <div className={`rounded-2xl p-4 ring-1 ${
                        ...
                        <p className={`text-xs font-semibold flex items-center gap-1.5 mb-1.5 ${
                          ...
                          {esIA ? t('advisory.aiAnswer') : t('advisory.agronomistAnswer')}
          ...
          <div
            ...
            <div className="rounded-t-[calc(2rem-0.375rem)] sm:rounded-[calc(1rem-0.375rem)] bg-white dark:bg-gray-900 max-h-[calc(92vh-0.75rem)] overflow-hidden flex flex-col">
              ...
              <div className="flex items-center justify-between px-5 py-4 border-b border-black/5 dark:border-white/10">
                <div>
                  <h2 className="font-display text-base font-bold tracking-tight text-gray-900 dark:text-white">{t('advisory.modalTitle')}</h2>
                  <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{t('advisory.modalSubtitle')}</p>
                </div>
              ...
              <div className="overflow-y-auto">
                <form onSubmit={handleSubmit} className="p-5 space-y-5">
                      ...
                      {ESPECIALIDADES.map(esp => (
                        <button key={esp.tipo} type="button"
                          ...
                          <span className={`text-xs font-semibold truncate ${form.tipo_asesoria === esp.tipo ? esp.text : 'text-gray-700 dark:text-gray-300'}`}>
                            {t(`advisory.specialties.${esp.tipo}.label`)}
                          </span>
                        </button>
                      ))}
                    </div>
                  ...
                  >
                    {preGroqLoading
                      ? <><div className="w-3.5 h-3.5 border-2 border-violet-400 border-t-transparent rounded-full animate-spin" /> {t('advisory.aiAnalysing')}</>
                      : <>{Ic.brain} {t('advisory.aiQuickHelp')}</>}
                  </button>
                  ...
                  {preGroq && (
                    <div className="bg-violet-50 dark:bg-violet-900/20 ring-1 ring-violet-200/60 dark:ring-violet-800/40 rounded-2xl p-4 animate-slide-up">
                      <p className="text-xs font-semibold text-violet-700 dark:text-violet-400 mb-2 flex items-center gap-1.5">
                        {Ic.brain} {t('advisory.aiPreliminary')}
                      </p>
                      <p className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed whitespace-pre-line">
                        {preGroq.replace(/\*\*(.*?)\*\*/g, '$1').replace(/\*/g, '')}
                      </p>
                      <p className="text-[10px] text-gray-400 mt-2">
                        {t('advisory.aiDisclaimer')}
                      </p>
                    </div>
                  )}
                  ...
                  <div className="flex gap-3 pt-1">
                    <button type="submit" disabled={submitting}
                      ...
                      {submitting
                        ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> {t('advisory.sending')}</>
                        : <>{Ic.send} {t('advisory.sendToExpert')}</>}
                    </button>
