...
const TIPOS = [
  { tipo:'Dashboard Ejecutivo', icono: Ic.trend,  color:'bg-blue-500',    light:'bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-800/40',         text:'text-blue-700 dark:text-blue-400'    },
  { tipo:'Estado de Cultivos',  icono: Ic.leaf,   color:'bg-emerald-500', light:'bg-emerald-50 dark:bg-emerald-900/20 border-emerald-200 dark:border-emerald-800/40',text:'text-emerald-700 dark:text-emerald-400'},
  { tipo:'Analisis Financiero', icono: Ic.dollar, color:'bg-amber-500',   light:'bg-amber-50 dark:bg-amber-900/20 border-amber-200 dark:border-amber-800/40',       text:'text-amber-700 dark:text-amber-400'   },
  { tipo:'Impacto Climatico',   icono: Ic.cloud,  color:'bg-sky-500',     light:'bg-sky-50 dark:bg-sky-900/20 border-sky-200 dark:border-sky-800/40',               text:'text-sky-700 dark:text-sky-400'       },
];
...
const esExcel = (formato) => String(formato || '').toUpperCase() === 'EXCEL';
...
const InformesPanel = () => {
  ...
  const handleGenerar = async () => {
    ...
    try {
      ...
      notify.error(err.response?.data?.message || t('reports.generateError'));
    } finally {
      setGenerating(false);
    }
  };
  ...
  const handleDescargar = async (inf) => {
    try {
      ...
      const ext  = esExcel(inf.formato) ? 'xlsx' : 'pdf';
      ...
      notify.success(t('reports.downloaded'));
    } catch {
      notify.error(t('reports.downloadError'));
    }
  };

  const totalPDF   = informes.filter(i => i.formato === 'PDF').length;
  const totalExcel = informes.filter(i => esExcel(i.formato)).length;
  const tipoDesc   = t(`reports.types.${genTipo}.desc`);
  ...
  return (
    <div className="px-4 sm:px-6 py-6 space-y-6 bg-gray-50 dark:bg-gray-950 min-h-full">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="font-display text-3xl sm:text-4xl font-bold tracking-tight text-gray-900 dark:text-white">{t('reports.title')}</h1>
          <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">{t('reports.subtitle')}</p>
        </div>
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            {[
              { icono: Ic.file,   val: estadisticas?.total_informes ?? 0,              lbl:t('reports.statsGenerated'),  color:'bg-blue-500'    },
              { icono: Ic.leaf,   val: estadisticas?.metricas?.total_cultivos ?? 0,    lbl:t('reports.statsCrops'),color:'bg-emerald-500'  },
              { icono: Ic.bug,    val: estadisticas?.metricas?.total_detecciones ?? 0, lbl:t('reports.statsDetections'),  color:'bg-red-500'      },
              { icono: Ic.file,   val: `${totalPDF}P / ${totalExcel}E`,                lbl:t('reports.statsFormats'), color:'bg-violet-500'   },
          ...
          <div className="rounded-2xl p-1.5 bg-white/60 dark:bg-white/[0.04] ring-1 ring-black/5 dark:ring-white/10">
            <div className="rounded-[calc(1rem-0.375rem)] bg-white dark:bg-gray-900 p-5 sm:p-6">
              <h2 className="font-display text-lg font-bold tracking-tight text-gray-900 dark:text-white">{t('reports.availableTypes')}</h2>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 mb-4">{t('reports.availableTypesHint')}</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {TIPOS.map(item => (
                  <button key={item.tipo} type="button" onClick={() => openModal(item.tipo)}
                    className={`group flex items-start gap-3 p-4 rounded-[1.25rem] ring-1 ring-black/5 dark:ring-white/10 text-left ${MOTION} hover:ring-emerald-400/50 active:scale-[0.99] touch-manipulation ${item.light}`}>
                    <div className={`${item.color} w-10 h-10 rounded-2xl flex items-center justify-center text-white shrink-0`}>{item.icono}</div>
                    <div className="min-w-0 flex-1">
                      <p className={`text-sm font-semibold tracking-tight ${item.text}`}>{t(`reports.types.${item.tipo}.label`)}</p>
                      <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 line-clamp-2">{t(`reports.types.${item.tipo}.desc`)}</p>
                    </div>
          ...
          <div className="rounded-2xl p-1.5 bg-white/60 dark:bg-white/[0.04] ring-1 ring-black/5 dark:ring-white/10">
            <div className="rounded-[calc(1rem-0.375rem)] bg-white dark:bg-gray-900 p-5 sm:p-6">
              <h2 className="font-display text-lg font-bold tracking-tight text-gray-900 dark:text-white">{t('reports.history')}</h2>
              <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 mb-4">{t('reports.historyHint')}</p>
              ...
              {informes.length === 0 ? (
                <div className="text-center py-12 rounded-[1.25rem] bg-gray-50 dark:bg-white/[0.03] ring-1 ring-black/5 dark:ring-white/10">
                  ...
                  <p className="text-sm font-semibold tracking-tight text-gray-700 dark:text-gray-300 mb-1">{t('reports.empty')}</p>
                  <p className="text-xs text-gray-400 dark:text-gray-500 mb-5">{t('reports.emptyHint')}</p>
                  <button onClick={() => openModal()}
                ...
                <div className="space-y-2.5">
                  {informes.map(inf => {
                    const fmtCfg = esExcel(inf.formato) ? FORMATO_CFG.EXCEL : FORMATO_CFG.PDF;
                    return (
                      <div key={inf.id} className={`flex items-center gap-3 p-4 rounded-[1.25rem] ring-1 ring-black/5 dark:ring-white/10 bg-white dark:bg-gray-900 ${MOTION} hover:ring-emerald-400/40`}>
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            <p className="text-sm font-semibold tracking-tight text-gray-900 dark:text-white truncate">{t(`reports.types.${inf.tipo_informe}.label`)}</p>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-end sm:items-center justify-center z-50 sm:p-4"
          ...
          <div className="rounded-t-[calc(2rem-0.375rem)] sm:rounded-[calc(1rem-0.375rem)] bg-white dark:bg-gray-900 flex flex-col max-h-[calc(92vh-0.75rem)]">
            <div className="flex items-center justify-between px-6 py-5 shrink-0">
              <div>
                <h2 className="font-display text-lg font-bold tracking-tight text-gray-900 dark:text-white">{t('reports.modalTitle')}</h2>
            ...
            {generating ? (
              <div className="flex-1 flex flex-col items-center justify-center gap-3 py-14 px-6">
                ...
                <p className="text-sm font-semibold tracking-tight text-gray-700 dark:text-gray-300">{t('reports.generating')}</p>
                <p className="text-xs text-gray-400 dark:text-gray-500 text-center max-w-xs">{t('reports.generatingHint')}</p>
              </div>
            ) : (
              <>
                <div className="flex-1 overflow-y-auto px-6 py-4 space-y-5 min-h-0">
                  ...
                  <div>
                    <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-2.5">{t('reports.reportType')}</p>
                    <div className="grid grid-cols-2 gap-2.5">
                      {TIPOS.map(item => (
                        <button key={item.tipo} type="button" onClick={() => setGenTipo(item.tipo)}
                          className={`flex flex-col gap-2 p-3.5 rounded-[1.25rem] ring-1 text-left ${MOTION} active:scale-[0.99] touch-manipulation ${
                            genTipo === item.tipo
                              ? `${item.light} ring-emerald-400`
                              : 'ring-black/5 dark:ring-white/10 hover:ring-black/10 dark:hover:ring-white/20'
                          }`}>
                          <div className="flex items-center justify-between">
                            <div className={`${item.color} w-8 h-8 rounded-2xl flex items-center justify-center text-white shrink-0`}>{item.icono}</div>
                            {genTipo === item.tipo && (
                              <span className="flex items-center justify-center w-5 h-5 rounded-full bg-emerald-500 text-white">{Ic.check}</span>
                            )}
                          </div>
                          <p className={`text-xs font-semibold leading-tight tracking-tight ${genTipo === item.tipo ? item.text : 'text-gray-800 dark:text-gray-200'}`}>
                            {t(`reports.types.${item.tipo}.label`)}
                          </p>
                        </button>
                      ))}
                    </div>
                  ...
                  <div>
                    <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-2.5">{t('reports.outputFormat')}</p>
                    <div className="flex gap-2.5">
                      {[
                        { fmt:'PDF',   label:'PDF',   sub:t('reports.pdfHint'), ext:'.pdf'  },
                        { fmt:'EXCEL', label:'Excel', sub:t('reports.excelHint'), ext:'.xlsx' },
                  ...
                  <div className="rounded-[1.25rem] px-4 py-3.5 bg-gray-50 dark:bg-white/[0.03] ring-1 ring-black/5 dark:ring-white/10">
                    <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1">{t('reports.willGenerate')}</p>
                    <p className="text-sm font-semibold tracking-tight text-gray-900 dark:text-white">{t(`reports.types.${genTipo}.label`)} — {genFormato}</p>
